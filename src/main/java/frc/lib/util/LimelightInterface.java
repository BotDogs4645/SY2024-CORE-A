...
public class LimelightInterface {
   ...
   * 
   * @return the distance from the robot to the target specified in the
   *         method's parameters.
   */
  public Optional<Double> calculateAbsoluteDistance(Optional<Pose3d> targetPose) {
   ...
   *                       the order of such and the "secondary" one does not
   *                       affect, in any way,
   *                       the functionality of
   *                       the method itself.
   * @param targetPosition - The position of the "secondary" location, although
   *                       the order of such and the "primary" one does not
   *                       affect, in any way, the
   *                       functionality of the method itself.
   * 
   * @return the distance which exists between the two locations specified
   ...
   * 
   * @return whether the position of the target specified in the method's
   *         parameters is "suitable" for use in trajectory/path generation.
   */
  public boolean validTargetInput(Optional<Translation3d> targetPosition) {
    ...
    if (targetPosition.isPresent() && currentRobotPosition.isPresent()
        && currentRobotPosition.get().getX() != targetPosition.get().getX()
        && currentRobotPosition.get().getY() != targetPosition.get().getY()) {
      return true;
    } else {
      return false;
    }
   ...
   * page 4.
   * 
   * @return the robot's estimated position in the game board.
   */
  public Optional<Pose3d> determinePosition() {
    Optional<Pose3d> targetOffset = getTargetPoseRobotRelative();
    ...
    Optional<Transform3d> originToTarget = Optional
        .of(Constants.Limelight.APRILTAGS.get(limelightInstance.getTagID().getAsInt()));
    if (originToTarget.isEmpty())
      return Optional.empty();
  ...
  /**
   * Calculates and @return the relative rotational offset to the given
  ...
  public Optional<Rotation2d> determineTargetRotationalOffset(Optional<Translation3d> targetPosition) {
    ...
    double xAngularOffset = (Math.atan2(yAxisOffset, xAxisOffset) * (180 / Math.PI))
        - currentRelativePosition.get().getRotation().getX();
    double yAngularOffset = Math
        .asin((targetPosition.get().getZ() - currentRelativePosition.get().getZ()) / calculatePlanarDistance(
            new Translation2d(currentRelativePosition.get().getX(), currentRelativePosition.get().getY()),
            new Translation2d(targetPosition.get().getX(), targetPosition.get().getY())).get());
